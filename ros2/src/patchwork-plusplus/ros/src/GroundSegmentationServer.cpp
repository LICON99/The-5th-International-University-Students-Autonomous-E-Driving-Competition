...
GroundSegmentationServer::GroundSegmentationServer(const rclcpp::NodeOptions &options)
  ...
  // ROI parameters
  roi_x_min_ = declare_parameter<double>("roi_x_min", roi_x_min_);
  roi_x_max_ = declare_parameter<double>("roi_x_max", roi_x_max_);
  roi_y_min_ = declare_parameter<double>("roi_y_min", roi_y_min_);
  roi_y_max_ = declare_parameter<double>("roi_y_max", roi_y_max_);
  roi_z_min_ = declare_parameter<double>("roi_z_min", roi_z_min_);
  roi_z_max_ = declare_parameter<double>("roi_z_max", roi_z_max_);
...
void GroundSegmentationServer::EstimateGround(
  ...
  // Apply ROI Filtering to nonground points
  nonground = ApplyROI(nonground);
...
Eigen::MatrixX3f GroundSegmentationServer::ApplyROI(const Eigen::MatrixX3f &cloud) {
  std::vector<int> indices;
  indices.reserve(cloud.rows());
  ...
  Eigen::MatrixX3f roi_cloud(indices.size(), 3);
  for (size_t i = 0; i < indices.size(); ++i) {
    roi_cloud.row(i) = cloud.row(indices[i]);
  }
